        ...
        return await withCheckedContinuation { continuation in
            CalendarService.shared.fetchTodaysEvents { events, error in
                if let error = error {
                    ...
                    continuation.resume(returning: ToolOutput("Sorry, I couldn't fetch your calendar events. Please check your Google Calendar connection."))
                    return
                }
                
                guard let events = events else {
                    Logging.general.log("GetTodaysCalendarEventsTool: No events returned")
                    continuation.resume(returning: ToolOutput("You have no calendar events scheduled for today."))
                    return
                }
                ...
                if events.isEmpty {
                    ...
                    continuation.resume(returning: ToolOutput("Today's calendar events (\(events.count) total):\n\(eventList)"))
                }
            }
        }
    }
